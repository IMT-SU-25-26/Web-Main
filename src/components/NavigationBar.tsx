...
export default function NavigationBar() {
  ...
  return (
    <nav className="z-[1000] fixed flex items-center px-12 justify-between w-screen min-h-[10vh] bg-[#fff] border-b-2 border-[#E93400] text-black">
      <Link
        href="/"
        className="w-[38px] sm:w-[32px] md:w-[52px] lg:w-[48px] xl:w-[48px] 2xl:w-[75px] h-auto cursor-pointer"
      >
        <Image
      ...
      <div
        className="flex lg:hidden flex-col gap-1 items-center cursor-pointer z-[1001]"
        onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
      >
        <div
          className={`w-7 h-1 bg-black transition-transform duration-300 ${
            mobileMenuOpen ? "rotate-45 translate-y-2" : ""
          }`}
        ></div>
        <div
          className={`w-7 h-1 bg-black transition-opacity duration-300 ${
      ...
      {mobileMenuOpen && (
        <div
          className="fixed top-[10vh] left-0 w-screen bg-slate-50/90 backdrop-blur-sm shadow-lg bg-opacity-50 z-[1000]"
